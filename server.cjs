...
app.post('/execute', (req, res) => {
  ...
  if (language === 'python') {
    ...
    command = 'docker';
    args = ['run', '--rm', '-i', 'gcc:latest', 'bash', '-c', 'cat > main.cpp && g++ main.cpp -o main && ./main'];
  ...
  child.on('error', (err) => {
    res.status(500).json({ error: "Failed to start Docker process. Is Docker running? Error: " + err.message });
  });
