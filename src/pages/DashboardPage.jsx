  ...
  const features = [
    {
      ...
      badge: "Voice & Webcam",
      color:
        "border-primary-500/20 bg-primary-500/5 hover:border-primary-500 text-primary-400",
    },
    {
      ...
      badge: "Career Tool",
      color:
        "border-emerald-500/20 bg-emerald-500/5 hover:border-emerald-500 text-emerald-400",
    },
    {
      title: "CodeHub",
      ...
      badge: "DSA Practice",
      color:
        "border-purple-500/20 bg-purple-500/5 hover:border-purple-500 text-purple-400",
    },
  ];
      ...
      <main className="flex-1 overflow-y-auto relative p-6 md:p-12 pt-20 md:pt-12">
        ...
        <div className="max-w-5xl mx-auto space-y-10 relative z-10 animate-fade-in">
          ...
          <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 border-b border-slate-800 pb-6">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold font-display tracking-tight bg-gradient-to-r from-white via-slate-100 to-slate-400 bg-clip-text text-transparent">
                Welcome back, {displayName}.
              </h1>
              <p className="text-slate-400 mt-2">
                Ready to master your next interview and level up?
              </p>
            </div>
          ...
          <section className="space-y-4">
            <h2 className="text-xl font-bold font-display tracking-tight text-slate-200">
              Interactive Career Suites
            </h2>
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="glassmorphism p-6 rounded-3xl border border-slate-800 flex flex-col justify-between">
                  ...
                  <div className="flex flex-wrap gap-2 max-h-[140px] overflow-y-auto">
                    {userProfile?.badges && userProfile.badges.length > 0 ? (
                      userProfile.badges.map((badge, idx) => (
                        <span
                          key={idx}
                          className="text-xs bg-slate-850 border border-slate-700/60 text-slate-350 px-3 py-1 rounded-xl font-medium shadow-sm flex items-center gap-1.5 hover:border-amber-400/50 transition-colors"
                        >
                          🏅 {badge}
                        </span>
                      ))
                ...
                <div className="pt-4 border-t border-slate-800/80 text-xs text-slate-400">
                  Earn more achievements by completing mock sessions, getting
                  scores above 80%, or solving code tasks.
                </div>
              </div>
            </section>
          )}
            ...
            <div className="glassmorphism rounded-3xl border border-slate-800 overflow-hidden">
              {loading ? (
                ...
                <div className="p-12 text-center text-slate-400 space-y-4">
                  ...
                  <p>
                    No past interviews found. Click "Interactive Interview" to
                    begin your journey!
                  </p>
                  <Link
                    to="/setup"
                    className="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-500 text-white px-5 py-2 rounded-xl text-sm font-medium transition-colors"
                  >
                ...
                <div className="divide-y divide-slate-800">
                  {pastInterviews.map((interview) => (
                    <div
                      key={interview.id}
                      className="p-6 hover:bg-slate-800/30 transition-colors flex items-center justify-between gap-4"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="font-medium text-slate-200">
                            ...
                            {interview.mode && (
                              <span className="ml-2 text-xs bg-slate-800 px-2 py-0.5 rounded text-primary-400 border border-slate-700/50">
                                {interview.mode}
                              </span>
                            )}
                          </h4>
                          <p className="text-sm text-slate-500 mt-1">
                            ...
                            Score: {interview.evaluation?.score || 0}/100 •
                            Trust:{" "}
                            {interview.cheatScore !== undefined
                              ? `${interview.cheatScore}%`
                              : "N/A"}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Link
                          ...
                          className="text-sm font-medium text-slate-350 hover:text-primary-400 transition-colors px-4 py-2 rounded-lg hover:bg-primary-500/10"
                        >
                          View
                        </Link>
                        <button
